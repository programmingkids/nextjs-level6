...
export default function Page() {
  ...
  const formAction = async (prevData: FormType, formData: FormData) => {
    // サーバアクションを起動
  ...
  // フォームの状態管理用フック呼び出し
  const [state, dispatch] = useFormState(formAction, defaultValue);
    ...
    <div className="main">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 lg:grid-cols-3 text-start">
        <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 my-4 col-start-1 md:col-start-2 lg:col-start-2 col-span-1 md:col-span-3 lg:col-span-1">
          ...
          {open && (
            <div className="mt-8">
              <div className="mb-4">Name: {state.name}</div>
              <div className="mb-4">Email: {state.email}</div>
            </div>
          )}
        </div>
      </div>
